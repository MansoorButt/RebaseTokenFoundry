...
DEPLOY_OUTPUT=$(forge create src/RebaseToken.sol:RebaseToken --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft --legacy --zksync)
echo "$DEPLOY_OUTPUT"
ZKSYNC_REBASE_TOKEN_ADDRESS=$(echo "$DEPLOY_OUTPUT" | grep -oP 'Deployed to: \K[0-9a-fA-F]+')
...
DEPLOY_OUTPUT=$(forge create src/RebaseTokenPool.sol:RebaseTokenPool --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft --legacy --zksync --constructor-args "$ZKSYNC_REBASE_TOKEN_ADDRESS" "[]" "$ZKSYNC_RNM_PROXY_ADDRESS" "$ZKSYNC_ROUTER")
echo "$DEPLOY_OUTPUT"
ZKSYNC_POOL_ADDRESS=$(echo "$DEPLOY_OUTPUT" | grep -oP 'Deployed to: \K[0-9a-fA-F]+')
echo "Pool address: $ZKSYNC_POOL_ADDRESS"

# Check if addresses were captured correctly
if [ -z "$ZKSYNC_REBASE_TOKEN_ADDRESS" ] || [ -z "$ZKSYNC_POOL_ADDRESS" ]; then
    echo "Error: Failed to capture contract addresses. Please check the deployment output."
    exit 1
fi

# Set the permissions for the pool contract
echo "Setting the permissions for the pool contract on ZKsync..."
cast send "$ZKSYNC_REBASE_TOKEN_ADDRESS" "grantMintandBurnRole(address)" "$ZKSYNC_POOL_ADDRESS" --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft
...
SEPOLIA_OUTPUT=$(forge script ./script/Deployer.s.sol:TokenAndPoolDeployer --rpc-url ${SEPOLIA_RPC_URL} --account updraft --broadcast)
echo "$SEPOLIA_OUTPUT"

# Extract the addresses from the output
SEPOLIA_REBASE_TOKEN_ADDRESS=$(echo "$SEPOLIA_OUTPUT" | grep -oP 'token: contract RebaseToken\s+\K[0-9a-fA-Fx]+')
SEPOLIA_POOL_ADDRESS=$(echo "$SEPOLIA_OUTPUT" | grep -oP 'pool: contract RebaseTokenPool\s+\K[0-9a-fA-Fx]+')
...
if [ -z "$SEPOLIA_REBASE_TOKEN_ADDRESS" ] || [ -z "$SEPOLIA_POOL_ADDRESS" ]; then
    echo "Error: Failed to capture Sepolia contract addresses. Please check the deployment output."
    exit 1
fi

# Deploy the vault 
echo "Deploying the vault on Sepolia..."
VAULT_OUTPUT=$(forge script ./script/Deployer.s.sol:VaultDeployer --rpc-url ${SEPOLIA_RPC_URL} --account updraft --broadcast --sig "run(address)" "$SEPOLIA_REBASE_TOKEN_ADDRESS")
echo "$VAULT_OUTPUT"
VAULT_ADDRESS=$(echo "$VAULT_OUTPUT" | grep -oP 'vault: contract Vault\s+\K[0-9a-fA-Fx]+')
echo "Vault address: $VAULT_ADDRESS"

# Check if the vault address was captured correctly
if [ -z "$VAULT_ADDRESS" ]; then
    echo "Error: Failed to capture vault address. Please check the deployment output."
    exit 1
fi

# Configure the pool on Sepolia - fixed formatting and quoting
...
cast send "$VAULT_ADDRESS" "deposit()" --value "$AMOUNT" --rpc-url ${SEPOLIA_RPC_URL} --account updraft

echo "Waiting for interest to accrue..."
sleep 30  # Wait 30 seconds for some interest to accrue

# Configure the pool on ZKsync - simplified approach
echo "Configuring the pool on ZKsync..."
# Break the complex typing into components
echo "Building the chain updates for ZKsync pool..."
...
# Use a simpler approach for the complex structure
forge script script/ConfigureZkSyncPool.s.sol:ConfigureZkSyncPoolScript --rpc-url ${ZKSYNC_SEPOLIA_RPC_URL} --account updraft --broadcast --sig "run(address,uint64,address,address)" "$ZKSYNC_POOL_ADDRESS" "$SEPOLIA_CHAIN_SELECTOR" "$SEPOLIA_POOL_ADDRESS" "$SEPOLIA_REBASE_TOKEN_ADDRESS"
